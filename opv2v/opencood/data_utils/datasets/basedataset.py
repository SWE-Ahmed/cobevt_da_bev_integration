...
class BaseDataset(Dataset):
    ...
    def reinitialize(self):
        ...
        for (i, scenario_folder) in enumerate(self.scenario_folders):
            ...
            # be ego vehicle.
            # Strip out any folder that starts with a hyphen (the RSUs)
            cav_list = [cav for cav in cav_list if not cav.startswith('-')]
            if int(cav_list[0]) < 0:
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                yaml_files = \
                    sorted([os.path.join(cav_path, x)
                            for x in os.listdir(cav_path) if
                            x.endswith('.yaml') and 'additional' not in x and 'gnss' not in x])
