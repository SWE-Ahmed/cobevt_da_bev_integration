...
def main():
    ...
    for i, batch_data in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            # Safely append 0.0 if the class is missing from the image
            static_ave_iou.append(iou_static[1] if len(iou_static) > 1 else 0.0)
            lane_ave_iou.append(iou_static[2] if len(iou_static) > 2 else 0.0)
            dynamic_ave_iou.append(iou_dynamic[1] if len(iou_dynamic) > 1 else 0.0)
